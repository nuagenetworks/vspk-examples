...
This script has the following features:
    * Assigns Policy Groups depending on regular expressions matching the VM Operating system in vCenter
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description="Script which will apply policy groups on VMs depending on the VM OS in vCenter for a certain set of Clusters")
    parser.add_argument('-c', '--cluster', required=True,
                        help='Cluster that has to be scanned for VMs (can be specified multiple times)',
                        dest='clusters', type=str, action='append')
    parser.add_argument('-d', '--debug', required=False, help='Enable debug output', dest='debug', action='store_true')
    parser.add_argument('-l', '--log-file', required=False, help='File to log to (default = stdout)', dest='logfile',
                        type=str)
    parser.add_argument('-m', '--mapping-file', required=True,
                        help='CSV file which contains the mapping of vCenter OS to policy groups', dest='mapping_file',
                        type=str)
    parser.add_argument('--nuage-enterprise', required=True,
                        help='The enterprise with which to connect to the Nuage VSD/SDK host', dest='nuage_enterprise',
                        type=str)
    parser.add_argument('--nuage-host', required=True, help='The Nuage VSD/SDK endpoint to connect to',
                        dest='nuage_host', type=str)
    parser.add_argument('--nuage-port', required=False,
                        help='The Nuage VSD/SDK server port to connect to (default = 8443)', dest='nuage_port',
                        type=int, default=8443)
    parser.add_argument('--nuage-password', required=False,
                        help='The password with which to connect to the Nuage VSD/SDK host. If not specified, the user is prompted at runtime for a password',
                        dest='nuage_password', type=str)
    parser.add_argument('--nuage-user', required=True,
                        help='The username with which to connect to the Nuage VSD/SDK host', dest='nuage_username',
                        type=str)
    parser.add_argument('-r', '--remove-policygroups', required=False,
                        help='Remove policygroups from all VMs before adding the correct matching ones.',
                        dest='remove_policygroups', action='store_true')
    parser.add_argument('-S', '--disable-SSL-certificate-verification', required=False,
                        help='Disable SSL certificate verification on connect', dest='nosslcheck', action='store_true')
    parser.add_argument('-v', '--verbose', required=False, help='Enable verbose output', dest='verbose',
                        action='store_true')
    parser.add_argument('--vcenter-host', required=True, help='The vCenter server to connect to, use the IP',
                        dest='vcenter_host', type=str)
    parser.add_argument('--vcenter-port', required=False,
                        help='The vCenter server HTTPS port to connect to (default = 443)', dest='vcenter_https_port',
                        type=int, default=443)
    parser.add_argument('--vcenter-password', required=False,
                        help='The password with which to connect to the vCenter host. If not specified, the user is prompted at runtime for a password',
                        dest='vcenter_password', type=str)
    parser.add_argument('--vcenter-user', required=True, help='The username with which to connect to the vCenter host',
                        dest='vcenter_username', type=str)
...
def update_nuage_policy_group(logger, nc, nc_vm_properties, nc_vm_pgs, remove_policygroups):
    ...
    if nc_domain is None:
        ...
        logger.error('Domain {0:s} can not be found in Nuage for VM {1:s}, skipping it'.format(nc_domain_name,
                                                                                               nc_vm_properties[
                                                                                                   'name']))
        return False
...
def main():
    ...
    if vcenter_password is None:
        ...
        vcenter_password = getpass.getpass(
            prompt='Enter password for vCenter host {0:s} for user {1:s}: '.format(vcenter_host, vcenter_username))
    ...
    try:
        ...
        if not nc or not nc.is_current_session():
            logger.error(
                'Could not connect to Nuage host {0:s} with user {1:s} and specified password'.format(nuage_host,
                                                                                                      nuage_username))
            return 1
        ...
        if not vc:
            logger.error(
                'Could not connect to vCenter host {0:s} with user {1:s} and specified password'.format(vcenter_host,
                                                                                                        vcenter_username))
            return 1
        ...
        for vc_vm in vc_vm_list:
            ...
            if vc_vm_nuage_enterprise is None or vc_vm_nuage_domain is None or vc_vm_nuage_zone is None or vc_vm_nuage_network is None:
                ...
                if vc_vm_nuage_enterprise is None or vc_vm_nuage_l2domain is None:
                    logger.info(
                        'VM {0:s} has no correct Nuage metadata set, assuming it is not a VM connected through Nuage and skipping it.'.format(
                            vc_vm.name))
                    continue
            ...
            logger.debug(
                'VM {0:s} with OS {1:s} has following Nuage settings: Enterprise {2:s}, Domain {3:s}, Zone {4:s}, Subnet {5:s}'.format(
                    nc_vm_properties['name'], nc_vm_properties['os'], nc_vm_properties['nuage.enterprise'],
                    nc_vm_domain_name, nc_vm_properties['nuage.zone'], nc_vm_properties['nuage.network']))

            # Getting VM MAC
            vc_vm_nic = next(
                (x for x in vc_vm.config.hardware.device if isinstance(x, vim.vm.device.VirtualEthernetCard)), None)
            ...
            if nc_vm_properties['vm_interface'] is None:
                logger.error('VM {0:s} with MAC address {1:s} is not known in Nuage, skipping it'.format(
            ...
            try:
                ...
                logger.error(
                    'VM {0:s} with MAC address {1:s} has a vm_interface but no vport in Nuage, this should not be possible... Skipping it'.format(
                        nc_vm_properties['name'], nc_vm_properties['mac']))
                continue

            logger.debug(
                'Found vm_interface and vport for VM {0:s} with MAC address {1:s}'.format(nc_vm_properties['name'],
                                                                                          nc_vm_properties['mac']))
            ...
            for regex in mapping_list.keys():
                ...
                if pattern.match(nc_vm_properties['os']):
                    logger.debug(
                        'Found match: regex "{0:s}" and VM OS "{1:s}", adding to the task list to hand over to Nuage.'.format(
                            regex, nc_vm_properties['os']))
